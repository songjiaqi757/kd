#!/usr/bin/env python3
"""A/B/C controlled visual-adaptation experiment. Run from any working directory."""
...
def parse_args():
    ...
    p.add_argument("--teacher-calibration-temperature", type=float, default=0.9259549975395203,
                   help="Frozen legacy D1 temperature for this controlled experiment; not refitted here")
...
def load_rows(args):
    ...
    subset = "ta" if args.mode == "ta_only" else "tav"
    targets = {}
...
def protocol(args):
    ...
    config.update(protocol="video-adaptation-v1", input_subset="ta" if args.mode == "ta_only" else "tav",
                  teacher_subset="ta" if args.mode == "ta_only" else "tav", video_frames=16,
                  alpha_ce=0.5, lambda_full=1.0, kd_temperature=2.0, gradient_accumulation=1,
                  checkpoint_selection="valid_mae", temperature_policy="fixed_legacy_d1_control",
    ...
    paths = [args.manifest, args.teacher_targets, Path(__file__),
...
def train(args, output):
    ...
    if args.diagnostics and args.mode != "ta_only":
        for perturbation in ("shuffle_video", "repeat_frame", "shuffle_frames"):
            loader = DataLoader(VideoDataset(valid_rows, video_processor, mode=args.mode,
                                             perturbation=perturbation, seed=args.seed + 200000), shuffle=False, **kwargs)
            dm, dr = evaluate(model, loader, device)
            diagnostics[perturbation] = {"metrics": dm, "delta_mae_vs_normal": dm["mae"] - metrics["mae"]}
            atomic_json(dr, output / f"diagnostic_{perturbation}.json")
...
def main():
    ...
    if args.dry_run:
        ...
        print(json.dumps({"mode": args.mode, "teacher_subset": "ta" if args.mode == "ta_only" else "tav",
