...
def main() -> int:
    ...
    if config.get("video_timing_policy") != "sampled_fps_v2":
        raise ValueError("timing-v2 feature assets required")
    ...
    if {r["split"] for r in rows} != {"train", "valid"}:
        raise ValueError("exactly train/valid required; test forbidden")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history.append(item)
        temporary = args.output / "history.json.tmp"
        temporary.write_text(json.dumps(history, indent=2) + "\n")
        temporary.replace(args.output / "history.json")
    ...
    report = {
        "schema_version": "teacher-probe-training-timing-v2",
        "seed": args.seed,
    ...
    (args.output / "report.json.tmp").write_text(
