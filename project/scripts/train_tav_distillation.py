#!/usr/bin/env python3
"""Unified online TAV M0--M6 trainer with immutable corrected teacher protocol."""
...
from rdid_mosei.video_adaptation import sample_video_frames
from rdid_mosei.tav_distillation import METHODS, TAVDistillationStudent, interaction_loss
...
def parse_args():
    ...
    p.add_argument("--assets", type=Path, default=ROOT / "outputs/experiments/tav_main_v1/assets/protocol.json")
    ...
    p.add_argument("--teacher-probe-report", type=Path)
    ...
    args.mode = "frozen_video" if args.method in ("M0", "M1", "M2") else "video_lora"
    if args.teacher_subset != "tav":
        p.error("M0--M6 require TAV teacher targets")
    ...
    if not math.isfinite(args.teacher_calibration_temperature) or abs(args.teacher_calibration_temperature - 0.9259549975) < 1e-6:
        p.error("legacy/nonfinite teacher temperature forbidden")
    if args.teacher_calibration_temperature != assets["teacher_temperature"]:
        p.error("calibration differs from frozen assets")
    return args
...
def load_rows(args):
    ...
    assets = json.loads(args.assets.read_text())
    target_path = Path(assets["interaction_targets"])
    if sha256(target_path) != assets["input_sha256"][str(target_path)]:
        raise ValueError("interaction target fingerprint differs")
    interaction_targets = {}
    for line in target_path.open():
        item = json.loads(line)
        key = item["parent_sample_id"]
        if key in interaction_targets:
            raise ValueError("duplicate interaction target")
        interaction_targets[key] = item
    for row in rows:
        item = interaction_targets[row["parent_sample_id"]]
        if item["split"] != row["split"] or item["sentiment"] != row["sentiment"] or item["video_id"] != row["video_id"]:
            raise ValueError("interaction identity mismatch")
        row["interaction_mean"] = item["mean"]
        row["interaction_variance"] = item["variance"]
...
def compute_loss(outputs, batch, device, args, assets):
    interaction = args.method in ("M4", "M5", "M6")
    output = outputs["tav"] if interaction else outputs
    ...
    kd, _, _ = weighted_full_kd_loss(output, batch["teacher_scores"].to(device),
                                    batch["teacher_logits"].to(device), w, 2.0,
                                    args.teacher_calibration_temperature, 1.0, 1.0)
    if not interaction:
        return task + kd
    mean = torch.tensor([r["interaction_mean"] for r in batch["rows"]], device=device)
    variance = torch.tensor([r["interaction_variance"] for r in batch["rows"]], device=device)
    utility = torch.tensor(assets["utility_normalized"], device=device)
    loss, _ = interaction_loss(outputs, mean, variance, utility, w,
                               assets["student_empty_baseline"], args.method)
    return task + kd + assets["lambda_interaction"] * loss
...
def protocol(args):
    ...
    config.update(protocol="tav-m0-m6-v1", input_subset="ta" if args.mode == "ta_only" else "tav",
                  ...
                  official_test_evaluated=False, torch_version=str(torch.__version__))
    ...
    paths.extend([args.assets, ROOT / "project/src/rdid_mosei/tav_distillation.py"])
    assets = json.loads(args.assets.read_text())
    for key in ("teacher_probe_seed", "teacher_temperature", "teacher_feature_manifest", "teacher_feature_checksum",
                "student_cache_manifest", "student_cache_checksum", "student_input_manifest", "student_input_checksum"):
        config[key] = assets[key]
    config["frozen_assets"] = assets
...
def build_model(args, device):
    ...
    return TAVDistillationStudent(text, audio, video, method=args.method, video_layers=args.video_layers,
...
def train(args, output):
    ...
    print(json.dumps({"status": "initialized", "mode": args.mode, "train_windows": len(train_rows),
                      "valid_windows": len(valid_rows), "method": args.method, "trainable_parameters": sum(p.numel() for p in parameters),
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
                predictions = model(**batch_inputs(batch, device), all_subsets=args.method in ("M4", "M5", "M6"))
                loss = compute_loss(predictions, batch, device, args, assets)
            ...
            if epoch == 1 and step == 1:
                adapter_grads = {n: float(p.grad.float().norm()) if p.grad is not None else None
                                 for n, p in model.named_parameters() if p.requires_grad and "lora_B" in n}
                if not all(v is not None and math.isfinite(v) and v > 0 for v in adapter_grads.values()):
                    raise RuntimeError("active LoRA B projection missing finite nonzero gradient")
                atomic_json(adapter_grads, output / "adapter_gradient_audit.json")
            optimizer.step()
        ...
        if metrics["mae"] < best_mae - 1e-5:
            ...
            atomic_save({"model": trainable_state(model), "epoch": epoch, "protocol": config}, output / "best.pt")
        else:
            stale += 1
        atomic_save({"model": trainable_state(model), "protocol": config, "optimizer": optimizer.state_dict(), "epoch": epoch,
    ...
    report = {"method": args.method, "protocol": config, "mode": args.mode, "seed": args.seed, "best_epoch": best_epoch, "epochs_run": len(history),
